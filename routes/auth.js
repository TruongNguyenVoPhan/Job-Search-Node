...
router.post("/forgotPassword", userValidator.checkChainn(), async function (req, res, next) {
  var result = validationResult(req);
  if (result.errors.length > 0) {
    ResHelper.RenderRes(res, false, result.errors);
    return;
...
router.post("/ResetPassword/:token", userValidator.checkChainnpass(), async function (req, res, next) {
  var result = validationResult(req);
  if (result.errors.length > 0) {
    ResHelper.RenderRes(res, false, result.errors);
    return;
...
router.post("/changePassword", checkLogin, userValidator.checkChainnpass(), async function (req, res, next) {
  var result = validationResult(req);
  if (result.errors.length > 0) {
    ResHelper.RenderRes(res, false, result.errors);
    return;
  ...
  if (user) {
    ...
    ResHelper.RenderRes(res, false, "Tài khoản không tồn tại");
  }
});
